...
v0.9 移除了 `L2DHeapsObject` 和 `L2DFlixelComponent` 的便捷委托方法。这些 wrapper 转发到 `L2DCore`，造成了重复的双 API。组件现在只暴露框架特定的生命周期（Flixel：`update`/`render`/`destroy`/`getSprite`；Heaps：`sync`/`onRemove`）和 transform getter（Flixel：`x`/`y`/`scale`/`alpha`）。所有行为都在 `L2DCore` 上 —— 通过 public `l2d.core` 字段访问。
...
以下委托方法已从 `L2DHeapsObject` 和 `L2DFlixelComponent` 中移除：
...
> **需重新编译 native：** v0.9 还向 `live2d_capi.h/cpp` 新增 8 个 C API（motion UserData 事件桥接 + part API）。现有的 `live2d_capi.dll` 和 `live2d_hl.hdll` 必须重新编译 —— 见 [第 3 步：编译 Native DLL](#第-3-步编译-native-dll-windows-x64)。
...
扩展层提供高层工具类，减少常见 Live2D 交互模式的样板代码。所有扩展均为**可选**——用户通过传入 `L2DCore` 引用构造扩展类即可使用。

**设计原则：** 依赖注入（接收 `L2DCore`，不继承）、接口优先（后端特定差异抽象为接口）、可组合、有状态 `update(dt)`。v0.8 扩展均为纯 Haxe，零 native 改动；v0.9 新增首个 native 端扩展（motion UserData 事件桥接），需要重新编译 `live2d_capi.dll` / `live2d_hl.hdll`。完整设计说明见 [ARCHITECTURE.md → Extension Layer (v0.8+)](./ARCHITECTURE.md#extension-layer-v08)。

> **v0.9 破坏性变更：** `L2DHeapsObject` 和 `L2DFlixelComponent` 不再暴露 motion/expression/hit-test/行为开关 API 的便捷委托。请改用 `l2d.core.startMotion(...)`、`l2d.core.setBreathEnabled(...)` 等。详见下方[破坏性变更 (v0.9)](#破坏性变更-v09)。
...
**动作优先级：** 当播放包含口型关键帧的动作时，可以通过 `motionActiveProvider` 让动作自带的口型参数优先于音频驱动的口型同步：

```haxe
lipSync.motionActiveProvider = () -> motionQueue.current != null;
```

当回调返回 `true` 时，`update()` 跳过外部口型值的设置，让动作的关键帧控制嘴巴。动作结束后口型同步自动恢复。
...
### L2DParts — Part 透明度链式 DSL + tween (v0.9)
...
// 链式 DSL —— set/show/hide/toggle 返回 handle 支持链式调用：
parts.part("PartHair").set(0.5).toggle();

// Tween（ease-in-out cubic，fire-and-forget —— manager 自动移除完成的 tween）：
parts.tween("PartHair", 0.0, 0.3); // 0.3 秒淡出

// 在 update 循环中：
parts.update(dt);

// 重置所有 part 到默认透明度（取消活动 tween）：
parts.reset();
```

### FlxL2DGroup — 多模型聚合 (v0.9, 仅 Flixel)
...
group.add(haru);
group.add(bg);
group.setHitAreas(haru, ["Head", "Body"]); // 每个 component 的 hit area 映射
add(group);

// 可选：自动鼠标 hit test + 拖拽（topmost-first）：
...
### L2DWavFileAudioSource — 纯 Haxe WAV 解码 + RMS (v0.9)

`IL2DAudioSource` 的 drop-in 实现，用纯 Haxe 解码 16-bit PCM WAV 文件并暴露滑动窗口 RMS 振幅。配合 `L2DLipSync` 从语音文件驱动口型，无需任何 native 音频依赖。

> **重要：** 此类只解码 WAV 数据并提供 RMS 振幅——它**不会播放音频**。实际音频输出需要用后端的音频 API（如 OpenFL `Sound`/`SoundChannel`、Heaps `hxd.snd.Channel`）。使用 `positionProvider` 将 RMS 计算同步到实际音频播放。
...
source.looping = false; // 默认: true
var lipSync = new L2DLipSync(core, source);
lipSync.enable();

// 在 update 循环中（推进播放头和 RMS 窗口）：
source.update(dt);
lipSync.update(dt);
```

**外部同步模式**（推荐——将 RMS 同步到实际音频播放）：
...
var source = new L2DWavFileAudioSource('assets/audio/voice_001.wav');
source.looping = true; // 与音频循环行为保持一致

// 通过后端播放音频（OpenFL 示例）
var sound = Sound.fromFile('assets/audio/voice_001.wav');

// 循环播放：OpenFL Sound.play 的 loops 参数在不同后端行为不一致。
// 使用 Event.SOUND_COMPLETE + 递归重播实现可靠循环。
function playLipSyncAudio():Void {
    var channel = sound.play();
    // 将 RMS 位置同步到实际音频播放（毫秒→秒）
    source.positionProvider = () -> channel.position / 1000.0;
    // 重新绑定 SOUND_COMPLETE 实现递归循环
...
// 在 update 循环中：
source.update(dt);     // 从 SoundChannel 同步位置
lipSync.update(dt);    // 读取 getAmplitude() → 驱动口型
```
...
### L2DDebugOverlay — Bounds/参数/HitArea 可视化 (v0.9)

抽象基类 + 后端特定子类：`L2DHeapsDebugOverlay`（Heaps）和 `L2DOpenFLDebugOverlay`（OpenFL/Flixel）。运行时切换 drawable bounds、参数值和 hit area 轮廓显示。
...
// 在 update 循环中（模型 update + render 之后）：
overlay.render();
```

### Motion UserData 事件 — Native 桥接 (v0.9)

Cubism motion 时间轴可以发出 UserData 字符串（例如 `"voice_001"` 用于触发音效）。v0.9 新增 native 端桥接：`LAppModel_CalcOnly` 重写 `MotionEventFired` 虚函数将事件收集到 per-model 队列，`L2DEventDispatcher.pollMotionEvents()` 每帧拉取队列。
...
// L2DMotionQueue.update(dt) 会在每帧开头自动调 dispatcher.pollMotionEvents()
// 如果不使用 MotionQueue，需手动调用：
dispatcher.pollMotionEvents();
```

> 需要重新编译 `live2d_capi.dll` 和 `live2d_hl.hdll` —— native 端新增 8 个 C API（`l2d_poll_motion_events` + 7 个 part API）。

### CLI — `haxelib run live2d-haxe` (v0.9)

```bash
# 从 model3.json 生成 Haxe 常量类（与 @:build 宏输出对齐）：
haxelib run live2d-haxe --gen-constants path/to/Haru.model3.json HaruConstants.hx HaruConstants

# 列出 model3.json 引用的所有资源文件：
...
| `motionActiveProvider` | 可选回调；返回 `true` 时跳过口型同步（动作优先） |
| `new(core, source)` | 用 L2DCore 和 `IL2DAudioSource` 构造 |
| `enable()` | 切换到外部口型同步模式（不会禁用 `lipSyncEnabled`） |
| `disable()` | 恢复 wav 文件处理模式 |
| `update(dt)` | 主循环更新——写入 `core.setLipSyncValue`（`motionActiveProvider` 返回 true 时跳过） |

#### L2DWavFileAudioSource

| 属性/方法 | 说明 |
| --- | --- |
| `looping` | 到末尾是否循环（默认 `true`） |
| `positionProvider` | 可选回调，返回秒数；设置后按外部音频源同步位置 |
| `finished` | 非循环模式下是否播放到末尾（只读） |
| `new(?path)` | 从文件路径加载（需 `sys`）；传 `null` 用于 `fromBytes` |
| `fromBytes(bytes)` | 静态方法：从内存 bytes 创建 |
| `update(dt)` | 推进或同步播放位置 |
| `getAmplitude()` | 滑动窗口 RMS 振幅 `[0, 1]` |
| `setWindowSize(n)` | 设置 RMS 窗口大小（帧数，默认 1024） |
| `rewind()` | 重置位置到开头 |
| `currentTime` | 当前播放位置（秒，只读） |
| `duration` | 总时长（秒，只读） |
...
> **v0.9：** motion/expression/hit-test/行为开关 API 的便捷委托已移除。请改用 `l2d.core.startMotion(...)`、`l2d.core.setBreathEnabled(...)` 等。组件只暴露框架特定的生命周期（`update`/`render`/`destroy`/`getSprite`）和 transform getter（`x`/`y`/`scale`/`alpha`）。

| 属性/方法 | 说明 |
| --- | --- |
| `core` | 底层 `L2DCore` —— 所有行为的唯一入口（motion/expression/hit-test/开关）。用它调 `startMotion`、`setExpression`、`hitTest`、`setDragging`、`setBreathEnabled` 等 |
| `x`, `y` | 屏幕坐标（委托到 `core.x`/`core.y`） |
| `scale` | 渲染缩放因子（委托到 `core.scale`） |
| `alpha` | 全局透明度乘数（委托到 `core.alpha`） |
...
> **v0.9：** motion/expression/hit-test/行为开关 API 的便捷委托已移除。请改用 `l2d.core.startMotion(...)`、`l2d.core.setBreathEnabled(...)` 等。

| 属性/方法 | 说明 |
| --- | --- |
| `core` | 底层 `L2DCore` —— 所有行为的唯一入口。用它调 `startMotion`、`setExpression`、`hitTest`、`setDragging`、`setBreathEnabled` 等 |
| `model` | 底层 `L2DModel` 句柄 |
| `modelDir`, `modelFileName` | 模型路径信息 |
| `modelWidth`, `modelHeight` | 计算得到的模型边界 |
